#!/usr/bin/env python3
"""
Schmidt-Samoa Cryptosystem Implementation - CORRECTED VERSION

This module implements the Schmidt-Samoa public-key cryptosystem with proper mathematical operations.
The security is based on the difficulty of factoring N = p²q.

Author: Claude (Fixed Implementation)
Date: August 2025

CRITICAL FIX: Corrected decryption algorithm to use proper Schmidt-Samoa mathematics
"""
...
class SchmidtSamoa:
    """Schmidt-Samoa Cryptosystem implementation with corrected mathematics."""
    ...
    def _decrypt_chunk(ciphertext_int, private_key, expected_length):
        """
        Decrypt a single chunk using CORRECTED Schmidt-Samoa decryption.
        
        The Schmidt-Samoa decryption uses the Chinese Remainder Theorem approach:
        - Compute c^d mod p²
        - Use the fact that N = p²q and the special structure
        ...
        # CORRECTED SCHMIDT-SAMOA DECRYPTION:
        # The proper decryption should use p² as the modulus, not just p
        p_squared = p * p
        
        # Compute c^d mod p²
        # This is the correct Schmidt-Samoa decryption operation
        m_candidate = pow(ciphertext_int, d, p_squared)
        
        # If the result is still too large, we need to reduce it further
        # In Schmidt-Samoa, the message should be recovered modulo p
        m = m_candidate % p
    ...
    def encrypt(message, public_key):
        ...
        try:
            ...
            if 'safe_chunk_size' in public_key:
                ...
                N = int(public_key['N'])
                safe_chunk_size = min(50, (N.bit_length() // 8) // 8)
    ...
    def decrypt(ciphertext_b64, private_key):
        """
        Decrypt ciphertext using CORRECTED Schmidt-Samoa cryptosystem.
...
if __name__ == "__main__":
    ...
    from key_generator_corrected import generate_schmidt_samoa_keys
    
    print("=== Schmidt-Samoa Cryptosystem Demo (CORRECTED) ===")
